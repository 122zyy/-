...
def GetFeature(StartTime,EndTime): 
    ...
    FRCorr = GetCorr(FRData.corr()) ;#计算并提取各基金对的fund_return相关性，即净值收益率相关性
    FRCumCor = GetCorr(FRData.cumsum(axis=1).corr())#计算并提取各基金对的fund_return累计值的相关性
    
    BRData = all_fund_benchmark_return[Date].T ;
    BRData = BRData.corr() ;
    BRCorr = GetCorr(BRData) ;#计算并提取各基金对的benchmark_return相关性，即基准收益率相关性
    ...
    TargetCor = (Target1FR.T).corrwith(Target2FR.T)#计算并提取各基金对fund_return相关性的相关性
